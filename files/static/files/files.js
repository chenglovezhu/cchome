...
// 文件大小格式化应用函数
function applyFileSizeFormat() {
    const fileSizeElements = document.querySelectorAll('.sizeFormat');
    if (fileSizeElements.length === 0) return; // 如果没有相关元素，直接返回

    fileSizeElements.forEach(function (fileSizeElement) {
        const fileSize = Number(fileSizeElement.textContent.trim());
        fileSizeElement.textContent = formatFileSize(fileSize);
    });
}

// 时间格式标准化函数
function standardizeTimeString(originalTime) {
    return originalTime
        .replace(/\bDec\./, "Dec")
        .replace(/\bJan\./, "Jan")
        .replace(/\bFeb\./, "Feb")
...
// 通用时间格式化函数
function formatTimeForElements(selector) {
    const elements = document.querySelectorAll(selector);
    if (elements.length === 0) return; // 如果没有相关元素，直接返回

    elements.forEach((element) => {
        const originalTime = element.textContent.trim();
        const standardizedTime = standardizeTimeString(originalTime);
        ...
        if (!isNaN(date)) {
            ...
            console.error("时间格式解析失败:", originalTime);
        }
    });
}
...
function validateForm() {
  ...
  if (!category || category === "选择分类......") missingFields.push("请选择分类");
...
async function previewFile(md5, mime) {
    //获取弹窗元素
    const modalBody = document.querySelector("#previewModal #previewContent");
    //定义从后端获取数据路径
    const url = `${window.location.origin}/files/data?md5=${md5}`;
    //获取文件信息（其中含文件块的数据链接）
    try {
      // 获取文件元数据
      ...
      if (data.status === "deleted") {
        modalBody.innerHTML = `<p class='h4'>文件已删除，无法预览！</p>`;
        // 显示模态框
      ...
      //如果视频已转换为HLS视频流，则优先采用HLS视频流播放
        ...
        content = `<video id="${md5}" controls class="w-100">HLS</video>`;
      }
  
      //先判断是否为大小<2G的视频，若为真，则开始下载视频数据，并创建加载对象
      else if (
        mime.startsWith("video/") &&
        data.fileSize < 2147483648 &&
      ...
      //先判断是否为大小>2G的视频，若为真，则提示文件过大，下载后再进行观看
      else if (
        mime.startsWith("video/") &&
        data.fileSize > 2147483648 &&
        !data.hlsAddr?.endsWith("m3u8")
      ...
      //若为其他类型，则提示无法预览该文件类型
        ...
        content = `<h2>无法预览该文件类型, 请下载后查看！</h2>`;
      }
...
async function editFileInfo(md5, name, album, subject, remark) {
  //将现有信息赋值到表单
...
async function deleteFile(md5) {
  ...
  const modalBody = document.querySelector("#previewModal .modal-body");
  // 构造获取文件元数据的 URL
  const url = `${window.location.origin}/files/delete?md5=${md5}`;
...
document.addEventListener('DOMContentLoaded', function () {
    applyFileSizeFormat(); // 调用文件大小格式化
    formatTimeForElements(".deleted-time"); // 格式化 .deleted-time 元素的时间
    formatTimeForElements(".created-time"); // 格式化 .created-time 元素的时间
});
